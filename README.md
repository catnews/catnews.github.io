...
- `DBLP`：计算机领域高质量元数据，利于追踪系统/网络顶会论文。
- `OpenReview`：前沿论文与评审讨论入口，适合发现新机制和早期创新。

## 数据来源（技术动态）

- `LWN.net`：优先抓取免费可读的内核深度文章。
- `Cloudflare Blog`：只保留明确命中 Linux 网络、eBPF/XDP 或网络性能路径的文章。
- `Cilium Blog`：聚焦 eBPF、Cilium 数据面与容器网络内核能力。
- `Phoronix` / `Kernel Newbies`：作为低数量补充源，仍需通过领域硬门槛。

说明：资讯抓取默认开启（可通过脚本配置关闭），只做少量高质量技术动态补充，不做泛资讯聚合；主流程优先保证论文与期刊研究质量。
...
- **中文总结兜底**：若 LLM 返回英文、空摘要或模板化短摘要，会结合标题、标签与英文摘要生成更具体的中文说明。
- **摘要质量约束**：论文摘要要求覆盖研究问题、核心方法、实验结论、Linux 内核网络关联与阅读关注点，避免只复述标题。
- **创新评分**：基于内核关键词、创新术语与实验评估信号计算 `innovationScore`（0-10）。
